...
- **A `BROWSER_TIMEZONE` the container's timezone data does not list, such as a misspelled `Europe/Stockholmm`, is now ignored with a warning naming it, instead of being handed to both browsers unchecked.** Chrome refused the bad zone and quietly kept the container's own, while the other engine was still given the value, so the timezone a site saw depended on which engine answered. The timezone falls back to following the exit IP.
...
- **A passthrough path like `/download.php?id=1`, and a page whose query string ends in `.js` or `.css`, are now fetched instead of refused.** The first was answered `403` because its first path segment contained a dot; the second `404` because the static-asset check looked at the query as well as the path. Any first path segment that is not an allow-listed host now routes to the default mirror, which is where the site's own links were always meant to go.
- **`PASSTHROUGH_TIMEOUT_MS` set to `0` or less now falls back to 60000ms instead of failing passthrough requests that arrive while another is being solved.** The solver already substituted its own default for such a value, so the request waiting behind it gave up before the solve it was waiting on could answer.
- **A proxy password survives no shape of the request now: a password carrying a `/`, `?` or `#`, a proxy URL with no scheme, and a proxy sent inside a list or a nested object all used to reach the log intact.** The parser those relied on reports no password where there plainly is one, so anything credential-shaped it cannot read is replaced whole. A failed timezone lookup also strips the percent-encoded form of the password, which is what the library quotes back.
- **A browser launch that fails while writing the proxy extension no longer leaves the username and password in a temp directory.**
- **`PROXY_URL` set but empty now says once in the log that requests are going out directly.** It stops every request failing, which was at least loud; a deployer who meant to proxy everything would otherwise never find out.
- **The log no longer records proxy passwords, cookie values or form field values; logs written by earlier versions may hold your `PROXY_PASSWORD`, so clear them or change it.** Every request was logged at the default level with the proxy password filled in from `PROXY_PASSWORD`, and debug logging added the solved cookies and the Turnstile token. A failed timezone lookup also wrote the proxy URL, credentials included, at the default level. The lines still show the command, the URL and every name, with the values replaced by `<redacted>`, including when a value arrives in a shape the API is about to refuse.
...
- **A solve on the Camoufox engine returns only the requested site's cookies, instead of every site the session has visited.** A session keeps one browser context alive, and the cookie jar was read whole, so a response could carry another site's clearance and login cookies. Clients add returned cookies by name without checking where they came from, so those then went out to the wrong site. The Chrome engine always returned the page's own.
- **An empty `PROXY_URL` now means no proxy, instead of failing every request with "Request parameter 'proxy' must have a 'url' field naming the proxy server."** Leaving the variable set but blank, which is what a compose file or an `.env` does, sent every request a proxy with no server in it.
- **A request now finishes inside its `maxTimeout` even when a browser has to be started for it, instead of overrunning by the launch time.** Starting Chrome or Camoufox takes seconds, and those seconds used to sit outside the budget: an engine got its full share to solve in on top of them, a Camoufox session launch was bounded only by `STEALTH_START_TIMEOUT` (120 seconds by default), and with both engines in play a request could take close to twice what was asked. The post-solve settle wait on the Camoufox engine is bounded by the same budget now, so a page that clears late comes back as a page rather than a timeout.
- **A Camoufox session whose browser fails to start now reports "Error solving the challenge." with the reason, instead of an empty message.** The failure happened before the engine's error handling began, so a launch timeout in particular came back as `Error: ` and nothing else. The Chrome engine always reported it properly.
- **Two requests naming one session no longer share its browser at the same time, which could answer one of them with the other's page.** They take the browser in turn now, on both engines, and a request that waits longer than its own `maxTimeout` for its turn is told so. The Camoufox engine already worked this way.
...
- **Pinning `BROWSER_TIMEZONE` no longer leaves the language half calling out to the exit IP; pin `LANG` too, or set `BROWSER_GEO`, for a deployment with no outbound access.** Those lookups sit inside the request's own `maxTimeout` and spend up to a minute failing on a host that cannot reach them, which is a minute the solve does not get.
- **A failed exit-IP lookup is retried a minute later instead of pinning the fallback timezone and language for the whole `SESSION_TTL_MINUTES` window.** A failure also no longer replaces an answer another request resolved while it was out, so one unreachable moment costs a minute rather than the rest of the window.
- **Two proxy accounts on one endpoint now resolve their own timezone and language, instead of sharing whichever was looked up first.** Residential providers select the exit country through the username, and the lookup was cached by proxy server alone, so switching account left the browser claiming the previous country until the cache expired.
- **A Chrome launch that fails before the browser starts no longer leaves the proxy username and password behind in a temp directory.** The cleanup only covered the launch itself, so a host with no Chrome installed, no display, or no matching driver failed earlier than that and left the credentials on disk.
- **With `RESPONSE_HEADERS=true`, `solution.headers` describes the page you asked for rather than an iframe on it.** A cleared Cloudflare page leaves its challenge iframe's response behind, and whichever document response came last was reported. The browser's network log is also emptied on every request now, so a long-lived session stops accumulating it when the headers are not reported (`returnOnlyCookies`, or a solve that failed).
- **A cookie sent with a `path` but no domain no longer fails the whole request on the Camoufox engine.** Cookies that do not say where they belong now take the requested site's host and apply to all of it, which is what the Chrome engine does with them; they were scoped to the requested URL's directory before, so a cookie sent for `/a/b` was not sent back for `/c`.
...
- **A solve that runs out of time no longer leaves its browser for the next request on that session.** The worker is stopped while its browser command may still be running, so the session's browser is closed and the next request builds a fresh one, on the same proxy. Without that, two requests could drive one browser and one could answer with the other's page.
- **A configured paid CAPTCHA solver no longer eats the free attempt on an ordinary budget.** The time reserved for it is only taken when what is left can fit both, so a request that cannot finish a paid call keeps its free one instead of getting neither.
- **A second engine that fails to start no longer throws away the page the first one returned.** Adding a fallback engine could make the answer worse than having none: the challenge page came back as an error instead.
- **The browser timezone is skipped when a slow lookup would leave the solve no time at all.** Resolving it for a proxy nobody has used yet can take tens of seconds, which used to come out of the same budget as the solve.
