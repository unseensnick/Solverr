...
- **An empty `PROXY_URL` now means no proxy, and says so once in the log, instead of failing every request with "Request parameter 'proxy' must have a 'url' field naming the proxy server."** Leaving the variable set but blank, which is what a compose file or an `.env` does, sent every request a proxy with no server in it. Traffic now leaves directly, which a deployer who meant to proxy everything would otherwise never find out.
...
- **The log no longer records proxy passwords, cookie values or form field values, in any shape they arrive in; logs written by earlier versions may hold your `PROXY_PASSWORD`, so clear them or change it.** Every request was logged at the default level with the proxy password filled in from `PROXY_PASSWORD`, and debug logging added the solved cookies and the Turnstile token. A failed timezone lookup wrote the proxy URL, credentials included, at the default level, and in the percent-encoded form the library quotes back. A password carrying a `/`, `?` or `#`, a proxy URL with no scheme, and a proxy sent inside a list or a nested object each defeated the parser the redaction relied on, so anything credential-shaped it cannot read is now replaced whole. The lines still show the command, the URL and every name, with the values replaced by `<redacted>`.
- **A browser launch that fails before the browser starts, or while writing the proxy extension, no longer leaves the proxy username and password in a temp directory.** The cleanup only covered the launch itself, so a host with no Chrome installed, no display or no matching driver failed earlier than that, and a failed write inside the extension directory left the credentials there with nobody holding the path.
- **Configuring a paid CAPTCHA solver no longer turns a hard challenge into a timeout, or eats the free attempt on an ordinary budget.** The escalation started only once the solve deadline had passed, with a couple of seconds left before the request had to answer, so it never finished. It has its own room inside the budget now, taken only when what is left can fit both, so a request too small for a paid call keeps its free one.
