...
def geo_ip_lookup_urls() -> list:
    ...
    Comma-separated, each an http(s) URL whose whole response body is the
    caller's IP. Added in front rather than replacing, so the services a
    ...
    for raw in os.environ.get('GEO_IP_LOOKUP_URLS', '').split(','):
        ...
        try:
            ...
            is_valid = parts.scheme in ('http', 'https') and bool(parts.hostname)
        except ValueError:
            is_valid = False
        if is_valid:
            urls.append(value)
        else:
            logging.warning("GEO_IP_LOOKUP_URLS entry %r is not an http(s) URL; ignoring it",
