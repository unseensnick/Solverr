...
- **Release state**, since sessions here often end near one: `git tag --sort=-v:refname | head -3`, the `version` in `package.json`, and whether the tag was pushed. If a release shipped, confirm both workflows went green (`gh run list -R unseensnick/Solverr --limit 4`, and note that `gh` resolves to the upstream remote without `-R`) and that `ghcr.io/unseensnick/solverr:latest` and the version tag report the same digests.
...
2. Run the browser-free tests if any code changed: `PYTHONPATH=src uv run --no-project python -m unittest test_detection test_response_shape`.
