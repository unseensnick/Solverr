...
- **playwright-captcha's own `eval` is blocked, in the click path as well as the paid one.** The click solver reaches into a Turnstile's closed shadow root with `evaluate_handle` (`solvers/click/common/shadow_root.py:53-55`), and the paid path injects its token with `page.evaluate` (`captchas/*/apply.py`). Both are main-world eval, and the Turnstile iframe's CSP blocks them: measured on 2026-08-08, every nudge logged "call to eval() blocked by CSP" and then "success element does not exist on the page". Neither the launch pref nor header stripping removes it (see the CSP divergence above), so a fix has to come from upstream running that traversal somewhere other than the main world.
