...
- `README.md` (the quick start) and `docs/*.md` (the user guides: installation, how it works, API, passthrough, configuration, language and timezone, troubleshooting): the user docs. A behavior or config change updates the matching guide. The version in `docs/api.md`'s response example moves at release-cut.
